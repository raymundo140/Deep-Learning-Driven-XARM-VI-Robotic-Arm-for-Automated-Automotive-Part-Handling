    ...
    def image_callback(self, msg):
        ...
        for cnt in contours:
            ...
            if area > 1000:
                ...
                # Count distinct white blobs
                white_contours, _ = cv2.findContours(white_inside, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

                if len(white_contours) >= 2 :  # change this threshold based on your object
                    object_found = True
                    M = cv2.moments(cnt)
                    if M['m00'] != 0:
                        cx = int(M['m10'] / M['m00'])
                        ...
                        cv2.putText(frame, f"✔️ White blobs: {len(white_contours)}", (cx - 60, cy - 10),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                    break

        if not object_found:
            self.get_logger().info("No valid black object with white dots found.")
